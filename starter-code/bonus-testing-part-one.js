...
function testRicksFaveAnimal() {
  assert (ricksFaveAnimal, 'ricksFaveAnimal exist.', 'ricksFaveAnimal is ' + typeof ricksFaveAnimal);
}
...
function testWhichAnimal() {
  ...
  assert(favoriteAnimals.indexOf(nextAnimal) >= 0, 'nextAnimal is valid.', 'nextAnimal is not valid.');
};
...
var randIdxNumber = Math.floor(Math.random() * (favoriteAnimals.length));
nextAnimal = favoriteAnimals[randIdxNumber];
testWhichAnimal();
